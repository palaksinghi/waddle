...
# # threshold LOWERED 0.1 -> 0.05 so smaller/faster steps still register
# # as valid "first contact" events instead of scoring negative every time
# #8
...
# # Angular velocity (roll/pitch rate) penalty -> stops head/torso wobble
# #13
# def ang_vel_xy_l2(ang_vel_b: np.ndarray) -> float:
#     return float(np.sum(ang_vel_b[:2] ** 2))

# # Constant per-step cost -> negative baseline forces exploration instead
# # of settling for "safe but useless" (e.g. standing still, or skating)
# #14
# def alive_cost() -> float:
#     return -1.0

# # Termination penalty — handled directly as float(terminated) in envm.py

# # ---------------------------------------------------------------------
# # FIX FOR "SKATING" EXPLOIT (confirmed at 3.35M steps):
# # ep_len_mean=1170, track_lin_vel_xy_exp=0.83, is_terminated=0 but
# # feet_air_time STILL NEGATIVE -- robot is sliding/skating forward on
# # the ground instead of actually stepping, because track_lin_vel_xy_exp
# # alone rewards forward motion regardless of HOW it's achieved.
# #
# # Changes:
# #   - track_lin_vel_xy_exp weight DOWN (6.0 -> 3.0): stop it from being
# #     the dominant, "cheapest to hack" term
# #   - gait_phase_contact turned back ON (0.0 -> 1.0): explicitly demands
# #     alternating single-foot contact, which skating cannot satisfy
# #   - feet_air_time weight UP (2.5 -> 5.0): make real stepping pay off
# #     more than skating ever could
# # ---------------------------------------------------------------------
# REWARD_WEIGHTS = {
#     "track_lin_vel_xy_exp": 3.0,     # DOWN from 6.0
#     "track_ang_vel_z_exp": 0.0,
#     "heading": 0.0,
#     "gait_phase_contact": 1.0,       # ON (was 0.0) -- forces alternating stance
#     "feet_air_time": 5.0,            # UP from 2.5 -- stepping must outweigh skating
...
# Gait phase / foot contact matching -- kept for future use, but weight
# is OFF right now (see REWARD_WEIGHTS note below)
#7
def gait_phase_contact_reward(t, period, left_contact: bool, right_contact: bool) -> float:
    ph = phase_vector(t, period)
    left_should_contact = ph[0] >= 0.0
...
# Feet air time (stepping reward)
# FIXED: previously this penalized ANY first_contact under the
# threshold, including tiny exploratory foot-lifts (a few ms of air
# time). That actively punished the policy for even attempting to
# step, which is why feet_air_time kept going negative no matter how
# long training ran. Now clipped at 0 -- short/failed attempts score
# neutral, only genuine steps that clear the threshold score positive.
#8
def feet_air_time(last_air_time: np.ndarray, first_contact: np.ndarray, cmd_xy: np.ndarray, threshold=0.05) -> float:
...
# ---------------------------------------------------------------------
# FIX (confirmed at 1.01M steps): feet_air_time still negative
# (-0.0015), gait_phase_contact flat at ~0.52 (= random chance level),
# action_rate_l2 RISING (18.6 -> 25.8, going the wrong way).
#
# Root cause: gait_phase_contact_reward uses a naive fixed 50/50 timer
# schedule completely decoupled from the robot's actual physics/speed,
# so it fights feet_air_time over WHEN to step -- two different signals
# telling the policy two different things about timing. Combined with
# feet_air_time's old unclipped penalty (which punished even small
# exploratory foot-lifts), the policy had no clean signal to learn from.
#
# Changes:
#   - feet_air_time() now clips at 0 -- never punishes an attempted
#     step, only rewards ones that clear the threshold
#   - gait_phase_contact weight -> 0.0 (was fighting feet_air_time)
# ---------------------------------------------------------------------
REWARD_WEIGHTS = {
    "track_lin_vel_xy_exp": 3.0,
    "track_ang_vel_z_exp": 0.0,
    "heading": 0.0,
    "gait_phase_contact": 0.0,       # OFF again -- was conflicting with feet_air_time
    "feet_air_time": 5.0,
    "flat_orientation_l2": -3.0,
    "base_height_l2": -0.5,
    "action_rate_l2": -0.01,
