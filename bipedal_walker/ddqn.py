...
CHECKPOINT_PATH = "ddqn_bipedal.pt"
...
'''
DQN Network (same architecture, used for both online and target nets in DDQN)
'''
...
print("Starting training (Double DQN)...")
for episode in range(1, num_episodes + 1):
    ...
    while not (terminated or truncated):
        ...
        if len(replay_buffer) >= batch_size:
            ...
            # 2) Use the TARGET network to evaluate that action's Q-value
            # This decouples action selection from action evaluation and
            # reduces the overestimation bias present in vanilla DQN.
            with torch.no_grad():
