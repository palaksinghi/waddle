...
class FrankaPandaGraspEnv:
    ...
    def step(self, actions):
        actions = torch.clip(actions, -self.env_cfg["action_range"], self.env_cfg["action_range"])
        ...
        final_obs, _ = self.get_observations()
        ...
        self.extras["final_observations"] = final_obs
...
def get_cfgs():
    env_cfg = {
        ...
        "action_range": 1.0,  # added from original
...
def get_env(num_envs: int) -> FrankaPandaGraspEnv:
    try:
        ...
        print(e)
    env_cfg, obs_cfg, reward_scales, robot_cfg = get_cfgs()
    env = FrankaPandaGraspEnv(
        num_envs=num_envs,
        env_cfg=env_cfg,
        obs_cfg=obs_cfg,
