...
def main():
    ...
    # -------------------------------------------------------------------
    # FIX: wrap the raw env in the SAME VecNormalize stats saved during
    # training. Without this, the policy sees obs on a completely
    # different scale than it was trained on -- this was why every
    # episode collapsed to the identical fall/reward/distance.
    # training_mode=False + norm_reward=False: we want normalized OBS
    # only (to match what the policy learned), raw reward for reporting.
    # -------------------------------------------------------------------
    ...
    # FIX: removed the duplicate second for-loop that was here before --
    # episodes were running twice, and the second copy referenced
    # start_x from the first loop's last iteration instead of its own.
    for ep in range(args.episodes):
        obs = vec_env.reset()
        if args.cmd is not None:
            env.cmd = np.array(args.cmd, dtype=np.float32)
        ...
        while not (terminated or truncated):
            ...
            terminated = bool(infos[0].get("terminated", dones[0]))
